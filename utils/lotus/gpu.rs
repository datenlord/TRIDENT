...
//#[link(name = "fpga", kind = "static")]
//#[link(name = "fpga")]
extern "C" {
    pub fn hash_on_fpga(preimages: *const c_uchar, hash_result: *mut c_uchar, batch_size: c_int, arity: c_int) -> c_int;
}
    ...
    A: Arity<Fr>,
{
    fn hash(&mut self, preimages: &[GenericArray<Fr, A>]) -> Result<Vec<Fr>, Error> {
        ...
        println!("Poseidon hashing on FPGA, batch_size = {}, arity = {}", batch_size, arity);
	    use std::time::Instant;
		let now = Instant::now();

		let mut frs = vec![<Fr as Field>::zero(); batch_size];

		unsafe {
			let _ret = hash_on_fpga(preimages.as_ptr() as *const u8, frs.as_ptr() as *mut u8, batch_size as i32, arity as i32);
		}

		let elapsed = now.elapsed();
		println!("Finished batch job, elapsed time: {:.2?}", elapsed);
